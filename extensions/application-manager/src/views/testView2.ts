...
export class TestView2 {
  constructor(context: vscode.ExtensionContext) {
    const view = vscode.window.createTreeView('testView2', { treeDataProvider: aNodeWithIdTreeDataProvider(), showCollapseAll: true });
    context.subscriptions.push(view);
    vscode.commands.registerCommand('testView2.reveal', async () => {
      const key = await vscode.window.showInputBox({ placeHolder: 'Type the label of the item to reveal' });
      if (key) {
        await view.reveal({ key }, { focus: true, select: false, expand: true });
      }
    });
    vscode.commands.registerCommand('testView2.changeTitle', async () => {
...
function aNodeWithIdTreeDataProvider(): vscode.TreeDataProvider<{ key: string }> {
  return {
    ...
    getTreeItem: (element: { key: string }): vscode.TreeItem => {
      ...
      treeItem.tooltip = `${element.key}message`;
      treeItem.iconPath = vscode.Uri.file('assets/dark/entry.svg');
      return treeItem;
    },
